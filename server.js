...
<style>
  ...
  *{box-sizing:border-box}
  body{margin:0;font-family:Poppins,system-ui,-apple-system,Segoe UI,Roboto,Ubuntu; background: radial-gradient(1200px 600px at 10% 10%, #1b2140 0%, #0b0f1a 60%), var(--bg); color:var(--text);}
  ...
  .big{font-size:26px}
  .pill{display:inline-block;padding:6px 10px;border-radius:999px;background:#101735;border:1px solid #253062;color:#b7c3e2;font-size:12px}
      ...
      <div class="card" id="createCard">
        ...
        <div class="row">
          ...
          <div>
            ...
            <select id="numQ">
              <option>5</option><option selected>10</option><option>15</option><option>20</option><option>25</option><option>30</option><option>40</option><option>50</option><option>75</option><option>100</option>
            </select>
          </div>
        </div>
        <div class="row">
...
async function api(path, method="GET", body=null){
  ...
  let data;
  try{ data = await r.json(); }catch{ throw new Error("Network/parse error"); }
...
createBtn.onclick = async ()=>{
  try{
    const playerName = (document.getElementById("p1name").value || "Player 1").trim();
...
joinBtn.onclick = async ()=>{
  try{
    const playerName = (document.getElementById("p2name").value || "Player 2").trim();
...
app.post("/api/answer", (req, res) => {
  ...
  if (player === "player2") {
    ...
    if (game.currentIndex >= game.settings.numQuestions) {
      game.finished = true;
      const { matches, total } = computeScore(game);
      leaderboard.push({
        code: game.code,
